#!/usr/bin/env bash
# Single-GPU inference server for DreamZero — AgiBot G1 embodiment (GB10 / any single CUDA device).
#
# Usage:
#   bash scripts/inference/serve_single_gpu_agibot.sh
#
...
# Defaults
# --------------------------------------------------------------------------- #
MODEL_PATH="${MODEL_PATH:-./checkpoints/DreamZero-AgiBot}"
PORT="${PORT:-8000}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Options:"
            echo "  --model-path PATH       Path to DreamZero AgiBot checkpoint (default: ./checkpoints/DreamZero-AgiBot)"
...
python "${REPO_ROOT}/serve_single_gpu_agibot.py" "${PYTHON_ARGS[@]}"
