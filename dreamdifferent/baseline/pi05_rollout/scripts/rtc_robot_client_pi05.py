...
def _build_robot_config(args: argparse.Namespace):
    if args.cameras_json:
        camera_specs = json.loads(args.cameras_json)
        cameras = {
            name: OpenCVCameraConfig(
                index_or_path=_coerce_camera_index(str(spec["index_or_path"])),
                width=int(spec.get("width", args.camera_width)),
                height=int(spec.get("height", args.camera_height)),
                fps=int(spec.get("fps", args.camera_fps or args.fps)),
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Optional JSON mapping camera names to OpenCV specs, e.g. "
            '\'{"front":{"index_or_path":0,"width":640,"height":480,"fps":30},'
            '"wrist":{"index_or_path":1,"width":640,"height":480,"fps":30}}\'. '
            "When set, this overrides --camera-name/--camera-index."
        ),
    )
    parser.add_argument(
        "--rename-map",
        default="{}",
        help="JSON mapping from runtime observation keys to policy keys. Usually {} if training used the same camera names.",
    )
