...
# Monkey-patch PreTrainedPolicy._load_as_safetensor to skip keys whose shapes
# differ from the model (e.g. action_in_proj / action_out_proj when max_action_dim
# is changed). Mismatched keys are dropped and left at their random-init values.
import lerobot.policies.pretrained as _pretrained

@classmethod
def _load_as_safetensor_skip_shape_mismatch(cls, model, model_file, map_location, strict):
    ...
    if skipped:
        logging.warning(
            f"Shape mismatch — skipping {len(skipped)} key(s), leaving them randomly initialised: {skipped}"
        )
