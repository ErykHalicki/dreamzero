...
class OpenPIWebsocketClient:
    ...
    def infer(self, observation: dict[str, Any], server_action_horizon: int) -> PolicyResult:
        ...
        actions_payload = result.get("actions", result.get(b"actions"))
        actions = np.asarray(actions_payload, dtype=np.float32)
        ...
        return PolicyResult(
            ...
            server_timing=dict(result.get("server_timing", result.get(b"server_timing", {}))),
        )
